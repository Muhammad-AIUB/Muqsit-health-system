...
// chosen, added as a related family member, or created brand-new. Used by the
// prescription header, the OPD queue form and the IPD admit form.

export const RELATIONS = ["Son", "Daughter", "Spouse", "Father", "Mother", "Brother", "Sister"];
export const cleanDigits = (v: string) => v.replace(/\D/g, "").slice(0, 11);

interface Props {
  value: string;
  onChange: (digits: string) => void;
  onPick: (p: Patient) => void;
  // Prefer this patient as the "Add person related to …" anchor when present.
...
export default function PatientMobileLookup({
  ...
  const pick = (p: Patient) => { onPick(p); setOpen(false); };
