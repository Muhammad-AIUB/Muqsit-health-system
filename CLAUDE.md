...
- **Multiple GitHub accounts on this machine, and the default is the WRONG one.** `git config user.email` on this repo has been found set to `unidevgo.qa1@gmail.com`; commits/pushes must be attributed to `Muhammad-AIUB <mjubayer.aiub20@gmail.com>`. Check **before the first commit**, not just before pushing — both `git config user.email` and `gh auth status` (active account). Re-authoring already-committed-but-unpushed commits: fix `git config user.name/user.email`, then `git rebase <base> --exec 'git commit --amend --reset-author --no-edit'`, then confirm the tree is unchanged (`git diff <tag-of-old-head> main` must be empty).
